...
// ★ここがポイント：citiesと関数をAppの外（上）に出したお！
const cities = [
  { name: 'Tokyo', label: '東京' },
  { name: 'Osaka', label: '大阪' },
...
const handleGetCurrentLocation = () => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          alert("位置情報の取得に失敗したお。設定を確認してみてね。");
        }
      );
    } else {
      alert("お使いのブラウザは位置情報に対応していないお。");
    }
  };
  ...
  return (
    <div style={{ 
      ...
      // ★ここを書き換えたお！背景色を動的に変更
      background: getBackgroundColor(weatherMain), 
      margin: 0, padding: '20px', boxSizing: 'border-box',
      transition: 'background 0.5s ease' // ふわっと変わるようにしたお
    }}>

      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <div style={{ 
          width: '100%', maxWidth: '500px', textAlign: 'center',
          // 画像があれば画像を表示、なければ透明にする
...
<button onClick={handleGetCurrentLocation} style={{ 
            ...
            background: '#fff',
            color: '#ff6b6b', fontWeight: 'bold', fontSize: '12px'
          }}>
            📍 現在地
          </button>
        ...
        <div style={{ marginTop: '40px', paddingBottom: '30px', textAlign: 'center', color: 'white', opacity: 0.7, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px' }}>
          <div style={{ display: 'flex', gap: '15px' }}>
            <a href="/clothing.html" style={{ color: 'white', fontSize: '12px', textDecoration: 'underline' }}>服装の目安</a>
            <a href="/wind.html" style={{ color: 'white', fontSize: '12px', textDecoration: 'underline' }}>風速や体感温度について</a>
            <a href="/privacy.html" style={{ color: 'white', fontSize: '12px', textDecoration: 'underline' }}>プライバシーポリシー</a>
          </div>
